...
//MARK:- DeleteAddressAPIProtocol
protocol DeleteAddressAPIProtocol {
    func deleteAddress(completion: @escaping (Result<DeleteAddressModel?, NetworkError>) -> Void)
}

//MARK:- DeleteAddressAPI
class DeleteAddressAPI: DeleteAddressAPIProtocol {
    ...
    func deleteAddress(completion: @escaping (Result<DeleteAddressModel?, NetworkError>) -> Void) {
        ...
        let url = "https://ahlanwashlan.com.sa/api/v2/user/shipping/delete/\(id!)"
        ...
        ApiServices.instance.ParseData(url: url, methodType: .get, parameters: nil, headers: headers) { (result) in
            completion(result)
        }
