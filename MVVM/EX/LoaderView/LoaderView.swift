...
extension UIView {
    func showHideView(view: UIView, heightOfSubView: NSLayoutConstraint, image: UIImageView, show: String, hide: String, heightBefor: Float,heightAfter: Float) {
        if view.isHidden{
            view.isHidden = false
            heightOfSubView.constant = CGFloat(heightAfter)
            view.fadeIn(view: view)
            image.image = UIImage(named: hide)
        } else{
            view.isHidden = true
            heightOfSubView.constant = CGFloat(heightBefor)
            view.fadeOut(view: view)
            image.image = UIImage(named: show)
        }
    }
    ...
    func fadeIn(view: UIView, duration: TimeInterval = 0.5,
      ...
      func fadeOut(view: UIView, duration: TimeInterval = 0.5,
                   delay: TimeInterval = 0.0,
                   completion: @escaping (Bool) -> Void = {(finished: Bool) -> Void in }) {
        UIView.animate(withDuration: duration,
                       delay: delay,
